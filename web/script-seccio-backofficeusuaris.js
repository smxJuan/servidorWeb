class ObjSeccioBackofficeUsuaris {
    ...
    async iniciaSeccio () {
        ...
        if (objRebut.resultat === 'ok') {
            ...
            codiHTML = codiHTML + '<th class="cabecera-tabla-nomobil" style="font-weight: bold;"></th>'
            codiHTML = codiHTML + '<th class="cabecera-tabla-nomobil" style="font-weight: bold;">ID</th>'
            codiHTML = codiHTML + '<th class="cabecera-tabla" style="font-weight: bold;">CORREO</th>'
            codiHTML = codiHTML + '<th class="cabecera-tabla" style="font-weight: bold;">NOMBRE</th>'
            codiHTML = codiHTML + '<th class="cabecera-tabla" style="font-weight: bold;">TIPO</th>'
            codiHTML = codiHTML + '<th class="cabecera-tabla" style="font-weight: bold;"></th>'
            codiHTML = codiHTML + '</tr>'
            for (cntUsuari = 0; cntUsuari < navegacio.dadesSeccio.length; cntUsuari = cntUsuari + 1) {
                valor = navegacio.dadesSeccio[cntUsuari]
                codiHTML = codiHTML + '<tr>'
                codiHTML = codiHTML + '<td class="td-nomobil"><img src="' + valor.imatge + '" width="50" /></td>'
                codiHTML = codiHTML + '<td class="td-nomobil" style="font-weight: bold;">' + '<center>' + valor.id + '</center>' + '</td>'
